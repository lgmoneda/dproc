...
DprocTokens = struct
    ...
    val allToks = [EOF, ASSIGN, KW_teste, TARROW, ARROW, RB, LB, COLON, SEMI, COMMA, RP, LP, KW_orelse, KW_andalso, KW_else, KW_then, KW_if, KW_val, KW_fun, KW_fn, KW_end, KW_in, KW_let]
...
fun exp_PROD_1_ACT (SEMI, cmdSequence, declSequence, SEMI_SPAN : (Lex.pos * Lex.pos), cmdSequence_SPAN : (Lex.pos * Lex.pos), declSequence_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos)) = 
  ( A.Sequence(declSequence @ cmdSequence ))
fun exp_PROD_2_ACT (cmdSequence, blockCmd, SEMI1, SEMI2, cmdSequence_SPAN : (Lex.pos * Lex.pos), blockCmd_SPAN : (Lex.pos * Lex.pos), SEMI1_SPAN : (Lex.pos * Lex.pos), SEMI2_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos)) = 
  ( A.Sequence(blockCmd :: cmdSequence) )
fun exp_PROD_4_ACT (blockDecl, declSequence, blockDecl_SPAN : (Lex.pos * Lex.pos), declSequence_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos)) = 
  ( A.Sequence(blockDecl :: declSequence) )
fun exp_PROD_6_ACT (blockDecl, blockDecl_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos)) = 
  ( blockDecl )
...
fun expsequence_PROD_2_ACT (blockExp, blockExp_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos)) = 
  ( [blockExp] )
fun blockExp_PROD_1_ACT (KW_else, KW_then, KW_if, opExp, blockCmd1, blockCmd2, KW_else_SPAN : (Lex.pos * Lex.pos), KW_then_SPAN : (Lex.pos * Lex.pos), KW_if_SPAN : (Lex.pos * Lex.pos), opExp_SPAN : (Lex.pos * Lex.pos), blockCmd1_SPAN : (Lex.pos * Lex.pos), blockCmd2_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos)) = 
...
fun blockCmd_PROD_1_ACT (ID, opExp, ASSIGN, ID_SPAN : (Lex.pos * Lex.pos), opExp_SPAN : (Lex.pos * Lex.pos), ASSIGN_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos)) = 
  ( A.Assign(A.Variable(ref (A.create_symbol ID)), opExp) )
...
fun simpleExp_PROD_2_ACT (CON_bool, CON_bool_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos)) = 
  ( A.BoolConstant(CON_bool) )
fun simpleExp_PROD_3_ACT (CON_string, CON_string_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos)) = 
  ( A.StringConstant(CON_string) )
fun simpleExp_PROD_4_ACT (ID, ID_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos)) = 
...
fun simpleExp_PROD_5_ACT (LP, RP, exp, LP_SPAN : (Lex.pos * Lex.pos), RP_SPAN : (Lex.pos * Lex.pos), exp_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos)) = 
  ( exp )
fun simpleExp_PROD_6_ACT (LB, RB, blockExp, COMMA, explist, LB_SPAN : (Lex.pos * Lex.pos), RB_SPAN : (Lex.pos * Lex.pos), blockExp_SPAN : (Lex.pos * Lex.pos), COMMA_SPAN : (Lex.pos * Lex.pos), explist_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos)) = 
...
fun exp_NT (strm) = let
      fun exp_PROD_1 (strm) = let
            ...
            in
              (UserCode.exp_PROD_1_ACT (SEMI_RES, cmdSequence_RES, declSequence_RES, SEMI_SPAN : (Lex.pos * Lex.pos), cmdSequence_SPAN : (Lex.pos * Lex.pos), declSequence_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos)),
                FULL_SPAN, strm')
            end
      fun exp_PROD_2 (strm) = let
            ...
            in
              (UserCode.exp_PROD_2_ACT (cmdSequence_RES, blockCmd_RES, SEMI1_RES, SEMI2_RES, cmdSequence_SPAN : (Lex.pos * Lex.pos), blockCmd_SPAN : (Lex.pos * Lex.pos), SEMI1_SPAN : (Lex.pos * Lex.pos), SEMI2_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos)),
                FULL_SPAN, strm')
            end
      ...
      fun exp_PROD_4 (strm) = let
            ...
            val FULL_SPAN = (#1(blockDecl_SPAN), #2(declSequence_SPAN))
            in
              (UserCode.exp_PROD_4_ACT (blockDecl_RES, declSequence_RES, blockDecl_SPAN : (Lex.pos * Lex.pos), declSequence_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos)),
                FULL_SPAN, strm')
            end
      ...
      fun exp_PROD_6 (strm) = let
            ...
            val FULL_SPAN = (#1(blockDecl_SPAN), #2(blockDecl_SPAN))
            in
              (UserCode.exp_PROD_6_ACT (blockDecl_RES, blockDecl_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos)),
                FULL_SPAN, strm')
            end
      in
        (case (lex(strm))
         of (Tok.ID(_), _, strm') =>
              tryProds(strm, [exp_PROD_2, exp_PROD_3, exp_PROD_5])
              ...
              (case (lex(strm'))
               of (Tok.ID(_), _, strm') =>
                    (case (lex(strm'))
                     of (Tok.SEMI, _, strm') =>
                          (case (lex(strm'))
                           of (Tok.ID(_), _, strm') => exp_PROD_1(strm)
                            | (Tok.RP, _, strm') => exp_PROD_6(strm)
                            | (Tok.EOF, _, strm') => exp_PROD_6(strm)
                            | (Tok.KW_decl(_), _, strm') =>
                                tryProds(strm, [exp_PROD_1, exp_PROD_4])
                            | _ => fail()
                          (* end case *))
                      | _ => fail()
                    (* end case *))
          ...
          | (Tok.CON_int(_), _, strm') => exp_PROD_3(strm)
          | (Tok.CON_string(_), _, strm') => exp_PROD_3(strm)
          | (Tok.LP, _, strm') => exp_PROD_3(strm)
          | (Tok.LB, _, strm') => exp_PROD_3(strm)
          | (Tok.KW_func(_), _, strm') => exp_PROD_3(strm)
          | (Tok.CON_bool(_), _, strm') => exp_PROD_3(strm)
          | _ => fail()
        (* end case *))
      end
and blockCmd_NT (strm) = let
      val (ID_RES, ID_SPAN, strm') = matchID(strm)
...
and blockExp_NT (strm) = let
      fun blockExp_PROD_1 (strm) = let
            ...
            in
              (UserCode.blockExp_PROD_1_ACT (KW_else_RES, KW_then_RES, KW_if_RES, opExp_RES, blockCmd1_RES, blockCmd2_RES, KW_else_SPAN : (Lex.pos * Lex.pos), KW_then_SPAN : (Lex.pos * Lex.pos), KW_if_SPAN : (Lex.pos * Lex.pos), opExp_SPAN : (Lex.pos * Lex.pos), blockCmd1_SPAN : (Lex.pos * Lex.pos), blockCmd2_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos)),
                FULL_SPAN, strm')
            end
      ...
      in
        (case (lex(strm))
         of (Tok.CON_int(_), _, strm') => blockExp_PROD_2(strm)
          | (Tok.CON_string(_), _, strm') => blockExp_PROD_2(strm)
          | (Tok.ID(_), _, strm') => blockExp_PROD_2(strm)
          | (Tok.LP, _, strm') => blockExp_PROD_2(strm)
          | (Tok.LB, _, strm') => blockExp_PROD_2(strm)
          | (Tok.KW_func(_), _, strm') => blockExp_PROD_2(strm)
          | (Tok.CON_bool(_), _, strm') => blockExp_PROD_2(strm)
          | (Tok.KW_if, _, strm') => blockExp_PROD_1(strm)
          | _ => fail()
        (* end case *))
      end
...
and simpleExp_NT (strm) = let
      ...
      fun simpleExp_PROD_2 (strm) = let
            ...
            in
              (UserCode.simpleExp_PROD_2_ACT (CON_bool_RES, CON_bool_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos)),
                FULL_SPAN, strm')
            end
      fun simpleExp_PROD_3 (strm) = let
            ...
            in
              (UserCode.simpleExp_PROD_3_ACT (CON_string_RES, CON_string_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos)),
                FULL_SPAN, strm')
            end
      fun simpleExp_PROD_4 (strm) = let
            ...
            in
              (UserCode.simpleExp_PROD_4_ACT (ID_RES, ID_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos)),
                FULL_SPAN, strm')
            end
      ...
      fun simpleExp_PROD_6 (strm) = let
            ...
            in
              (UserCode.simpleExp_PROD_6_ACT (LB_RES, RB_RES, blockExp_RES, COMMA_RES, explist_RES, LB_SPAN : (Lex.pos * Lex.pos), RB_SPAN : (Lex.pos * Lex.pos), blockExp_SPAN : (Lex.pos * Lex.pos), COMMA_SPAN : (Lex.pos * Lex.pos), explist_SPAN : (Lex.pos * Lex.pos), FULL_SPAN : (Lex.pos * Lex.pos)),
                FULL_SPAN, strm')
            end
      ...
      in
        (case (lex(strm))
         of (Tok.LP, _, strm') =>
              (case (lex(strm'))
               of (Tok.KW_if, _, strm') => simpleExp_PROD_5(strm)
                | (Tok.CON_int(_), _, strm') => simpleExp_PROD_5(strm)
                | (Tok.CON_string(_), _, strm') => simpleExp_PROD_5(strm)
                | (Tok.ID(_), _, strm') => simpleExp_PROD_5(strm)
                | (Tok.LP, _, strm') => simpleExp_PROD_5(strm)
                | (Tok.LB, _, strm') => simpleExp_PROD_5(strm)
                | (Tok.KW_decl(_), _, strm') => simpleExp_PROD_5(strm)
                | (Tok.KW_func(_), _, strm') => simpleExp_PROD_5(strm)
                | (Tok.CON_bool(_), _, strm') => simpleExp_PROD_5(strm)
                | (Tok.RP, _, strm') => simpleExp_PROD_7(strm)
                | _ => fail()
              (* end case *))
          | (Tok.CON_string(_), _, strm') => simpleExp_PROD_3(strm)
          | (Tok.CON_int(_), _, strm') => simpleExp_PROD_1(strm)
          | (Tok.CON_bool(_), _, strm') => simpleExp_PROD_2(strm)
          | (Tok.ID(_), _, strm') => simpleExp_PROD_4(strm)
          | (Tok.LB, _, strm') => simpleExp_PROD_6(strm)
          | _ => fail()
        (* end case *))
      end
and cmdSequence_NT (strm) = let
